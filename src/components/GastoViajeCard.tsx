...
export default function GastoViajeCard({id_gasto_viaje, valor, tipo_gasto}:GastoViaje) {
  return (
    <>
    ...
    <div className="flex justify-between  bg-white p-2  rounded-lg shadow-md border-1 border-slate-100 hover:shadow-lg transition-shadow duration-200 lg:ml-4 lg:mr-8 hover:bg-slate-200">
        <div className=" flex flex-col gap-y-2 ml-4">
            ...
            <span className="rounded-lg p-1 bg-slate-200 text-green-800 font-semibold" >  {new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(valor)}</span>
        </div>

        <div className="  flex items-center rounded-lg hover:shadow-lg transition-shadow duration-200 p-4 cursor-pointer hover:bg-slate-200 mr-4">
